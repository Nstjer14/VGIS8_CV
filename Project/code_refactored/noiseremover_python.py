...
import copy
'''
This contains two functions, equalisehistogram and noiseremover.
They are used to remove the eyelashes from and reconstruct the removed iris.
'''

#@profile
def equalisehistogram(reconstructIris,LimitValue):
    '''
    equalisehistogram(reconstructIris,LimitValue) takes an normalised iris image
    and performs histogram equalisation on it.
    reconstructIris: the image that needs to me equalised
    LimitValue: I don't know
    '''
    numberOfBins=256
...
def noiseremover(sourceimage,HistoFrac,RecognitionValue): 
    '''
    This removes the eyelashes on the assumpsion that the lowest value pixels
    are the eyelashes. They are then reconstructed by using 4-connectivity. It
    looks at the 4 neighbours and checks makes the new pixel an average of 
    those as long as there are atleast two non zero pixels.
    '''
    numberOfBins=256
